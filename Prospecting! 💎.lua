local Fluent = loadstring(game:HttpGet("https://raw.githubusercontent.com/BrainlessCody/Dl2sVlhwSe/refs/heads/main/Fluent.lua"))()
local SaveManager = loadstring(game:HttpGet("https://raw.githubusercontent.com/BrainlessCody/Dl2sVlhwSe/refs/heads/main/Addons/SaveManager.lua"))()
local InterfaceManager = loadstring(game:HttpGet("https://raw.githubusercontent.com/BrainlessCody/Dl2sVlhwSe/refs/heads/main/Addons/InterfaceManager.lua"))()
...
local walkToMerchantEnabled = false

-- Caminhar até o mercador com Pathfinding
local function walkTo(merchant)
	local char = LocalPlayer.Character or LocalPlayer.CharacterAdded:Wait()
	local hrp = char:WaitForChild("HumanoidRootPart")
	local humanoid = char:FindFirstChildOfClass("Humanoid")
	if not humanoid or not merchant then return end

	local mHRP = merchant:FindFirstChild("HumanoidRootPart")
	if not mHRP then return end

	local destination = mHRP.Position
	...
	if path.Status == Enum.PathStatus.Success then
		for _, waypoint in ipairs(path:GetWaypoints()) do
			-- Se estiver perto o suficiente, para e vende
        ...
        if state then
            task.spawn(function()
                while holdingPanAutoFarm do
                    ...
                    -- Venda após shake (respeita WalkToMerchant)
                    if sellAfterShake and holdingPanAutoFarm then
                        ...
                        if merchant and hrp then
                            if walkToMerchantEnabled and sellAfterShake then
                                -- Caminha até o mercador
                                walkTo(merchant)
                            else
                                -- TP por steps até o mercador
                            ...
                            -- Voltar para posição salva na areia
                            if savedPositionDC and walkToMerchantEnabled then
                                -- Caminha de volta para areia
                                local dummy = Instance.new("Model")
                                local dummyHRP = Instance.new("Part")
                                dummyHRP.Name = "HumanoidRootPart"
                                dummyHRP.Anchored = true
                                dummyHRP.Position = savedPositionDC.Position
                                dummyHRP.Parent = dummy
                                walkTo(dummy)
                                dummy:Destroy()
                            elseif savedPositionDC then
                                -- TP de volta
...
local sellAllSection = Tabs.Main:AddSection("💰 ‣ Sell By Inventory")

local PathfindingService = game:GetService("PathfindingService")
local Backpack = LocalPlayer:WaitForChild("Backpack")
local ToggleLockRemote = ReplicatedStorage:WaitForChild("Remotes"):WaitForChild("Inventory"):WaitForChild("ToggleLock")

local sellAllStep1 = 10
local inventoryLimit = 500


-- Função para buscar o mercador mais próximo
local function getClosestMerchant()
	local char = LocalPlayer.Character or LocalPlayer.CharacterAdded:Wait()
	local hrp = char and char:FindFirstChild("HumanoidRootPart")
	if not hrp then return nil end
	...
	local regions = {
		npcs:FindFirstChild("StarterTown"),
		npcs:FindFirstChild("RiverTown"),
		npcs:FindFirstChild("Cavern"),
	}

	local closest, minDist = nil, math.huge
	for _, region in ipairs(regions) do
		local merchant = region and region:FindFirstChild("Merchant")
		local mHRP = merchant and merchant:FindFirstChild("HumanoidRootPart")
		if mHRP then
...
-- Toggle: Caminhar até o mercador (ativa/desativa)
Tabs.Main:AddToggle("WalkToMerchant", {
	Title = "Walk to Merchant",
	Description = "Walk instead of teleporting to the merchant.",
	Default = false,
	Callback = function(state)
		walkToMerchantEnabled = state
	end
})

-- Toggle: Auto Sell All por inventário cheio
Tabs.Main:AddToggle("AutoSellAllByInventory", {
	Title = "Auto Sell All by inventory",
	Default = false,
	Callback = function(state)
		if state then
			task.spawn(function()
				while state do
					...
					for _, item in ipairs(Backpack:GetChildren()) do
						local type = item:GetAttribute("ItemType")
						if type == "Equipment" or type == "Valuable" then
							count += 1
						end
					end

					if count >= inventoryLimit and hrp then
						local merchant = getClosestMerchant()

						if merchant and merchant:FindFirstChild("HumanoidRootPart") then
							local dest = merchant.HumanoidRootPart.Position

							if walkToMerchantEnabled then
								walkTo(merchant)
								...
								-- Caminha de volta para savedPositionDC
								if savedPositionDC then
									local fakeMerchant = {
										FindFirstChild = function(_, _) return {
											Position = savedPositionDC.Position
										} end
									}
									walkTo(fakeMerchant)
								end
								...
								-- Teleporte por steps
								local step = sellAllStep1
								local pos = hrp.Position
								local dir = (dest - pos).Unit
								local dist = (dest - pos).Magnitude

								while dist > step do
									pos += dir * step
									hrp.CFrame = CFrame.new(pos, dest)
									task.wait(0.05)
									dist = (dest - pos).Magnitude
								end
								hrp.CFrame = CFrame.new(dest)
								task.wait(0.3)
								...
								-- Volta via steps
								if savedPositionDC then
									local destBack = savedPositionDC.Position
									local posBack = hrp.Position
									local dirBack = (destBack - posBack).Unit
									local distBack = (destBack - posBack).Magnitude

									while distBack > step do
										posBack += dirBack * step
										hrp.CFrame = CFrame.new(posBack, destBack)
										task.wait(0.05)
										distBack = (destBack - posBack).Magnitude
									end
...
-- Input: Definir limite de inventário
Tabs.Main:AddInput("InventorySellLimit", {
	...
	Finished = true,
	Callback = function(value)
		inventoryLimit = tonumber(value) or inventoryLimit
	end
})

-- Slider: Tamanho do passo de TP
Tabs.Main:AddSlider("SellAllStepSlider", {
	Title = "Sell All (Steps)",
	Description = "Ajuste do passo de TP para vender.",
	Min = 1,
	Max = 50,
	Default = 10,
	Rounding = 0,
...
Tabs.Main:AddToggle("FillPanToggle", {
    Title = "Fill Pan",
    Description = "Hold mouse click and collect until full",
    Default = false,
    Callback = function(state)
        holdingPan = state
        ...
        if state then
            task.spawn(function()
                ...
                while holdingPan do
                    ...
                    if tool and tool:GetAttribute("Fill") < LocalPlayer.Stats:GetAttribute("Capacity") then
...
Tabs.Main:AddToggle("ShakePanToggle", {
    Title = "Shake Pan",
    Description = "Shake the pan while Panning is active",
    Default = false,
    Callback = function(state)
        shaking = state
        ...
        if state then
            task.spawn(function()
                ...
                while shaking and char:GetAttribute("Panning") do
